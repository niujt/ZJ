...
public class PaperController {
    ...
    public ModelAndView paperList(HttpServletRequest request,@RequestParam(value = "currentPage",required = false)Integer currentPage,@RequestParam(value = "name",required = false)String name){
        if(currentPage==null){
            currentPage=1;
        }
        if(name!=null) {
        ...
        return new ModelAndView ("admin/PaperManagement");
    }
    ...
    public ModelAndView paperInfo(HttpServletRequest request, @PathVariable Integer id){
        request.setAttribute("paperInfo",service.getPaperById(id));
        return new ModelAndView("admin/info/PaperInfo");
    }
    ...
    public ModelAndView getAnswer( @PathVariable Integer id,HttpServletRequest request){
        request.setAttribute("answers",service.getPaperById(id).get("answers"));
        return new ModelAndView("paper/Answer");
    }
